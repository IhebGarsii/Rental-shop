...
function CarDetail() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery({ queryKey: ["carDetail"], queryFn: () => getCar(id) });

  useEffect(() => {
    if (!isLoading && !isError && car) {
      localStorage.setItem("idCar", car._id);
    }
  ...
  const { mutate: mutateDelete } = useMutation({
    ...
    onSuccess: () => {
      toast.success("Car deleted with succes");
      navigate("/Cars");
    },
    onError: (error) => {
      toast.error("error in deleteing the car", error);
    },
  });
            ...
            <div className="admin-car">
              ...
              {localStorage.getItem("roles") === "ADMIN" && (
                <div className="car-detail-action">
                  ...
                  <Delete onClick={() => mutateDelete(car._id)} />
                </div>
              )}
            </div>
